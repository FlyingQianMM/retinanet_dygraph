...
def optimizer_setting(learning_rate, boundaries, values, devices_num):
    learning_rate = cfg.learning_rate / 8.0 * devices_num
    warm_up_iter = cfg.warm_up_iter * 8 / devices_num
    start_lr = learning_rate * cfg.warm_up_factor
    
    boundaries = [int(b * 8 / devices_num) for b in boundaries]
    values = [v / 8.0 * devices_num for v in values]
    
    lr = fluid.layers.piecewise_decay_with_linear_lr_warmup(
        boundaries=boundaries,
        values=values,
        warmup_iter=warm_up_iter,
        start_lr=start_lr,
...
def train():
    ...
    optimizer = optimizer_setting(learning_rate, boundaries, values, devices_num)
    ...
    if cfg.pretrained_model:
        ...
        pretrained, optimizer_load = fluid.dygraph.load_persistables(cfg.pretrained_model)
    ...
    def train_loop():
        ...
        for iter_id, data in enumerate(train_reader()):
            ...
            if (iter_id + 1) == cfg.max_iter:
                break
    train_loop()
    save_model(retinanet.state_dict(), 'model_final', optimizer)
