...
class add_fpn_neck(fluid.dygraph.Layer):
    ...
    def forward(self, inputs):
        ...
        for i in range(self.start_level, self.end_level):
            ...
            #print('fpn lateral_{} {} {}'.format(i+3, abs(conv.numpy()).sum(), conv.numpy().shape))
        ...
        for i in range(used_body_levels - 1, 0, -1):
            ...
            top_down = fluid.layers.resize_nearest(lateral_convs[i],
                                                   ...
                                                   name=topdown_name)
            #print('fpn_inner_{}_topdown {} {}'.format(self.inputs_name[1-i], abs(top_down.numpy()).sum(), top_down.numpy().shape))
            lateral_convs[i - 1] = fluid.layers.elementwise_add(
                x=lateral_convs[i - 1],
                y=top_down,
                name=fpn_inner_name
                )
            #print('fpn_inner_{} {} {}'.format(self.inputs_name[1-i], abs(lateral_convs[i-1].numpy()).sum(), lateral_convs[i-1].numpy().shape))
        ...
        for i in range(used_body_levels):
            ...
            #print('fpn_{} {} {}'.format(self.inputs_name[i], abs(conv.numpy()).sum(), conv.numpy().shape))
            outs.append(conv)
        ...
        if(self.num_outs > used_body_levels):
            ...
            #print('fpn_6 {} {}'.format(abs(fpn_blob.numpy()).sum(), fpn_blob.numpy().shape))
            fpn_blob = fluid.layers.relu(fpn_blob)
            fpn_blob_in = fpn_blob
            fpn_blob = self.out_conv7(fpn_blob_in)
            outs.append(fpn_blob)
            #print('fpn_7 {} {}'.format(abs(fpn_blob.numpy()).sum(), fpn_blob.numpy().shape))
