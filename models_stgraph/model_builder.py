...
class RetinaNet(object):
    ...
    def eval_bbox(self):
        ...
        self.pred_result = fluid.layers.retina_detection_out(
           bboxes = boxes_list,
           scores = scores_list,
           anchors = anchor_list,
           ...
           nms_eta=cfg.TEST.eta)
        '''
        self.pred_result = fluid.layers.retinanet_detection_output(
            bboxes=boxes,
            scores=scores,
            anchors=anchors,
            im_info = self.im_info,
    ...
    def loss(self):
        ...
        for i in range(len(self.cls_score)):
            ...
            cls_score_reshape = fluid.layers.reshape(
                x=cls_score_reshape, shape=(0, -1, cfg.class_num - 1), name='cls_score_reshape_fpn{}'.format(i+3))
            bbox_pred_reshape = fluid.layers.reshape(
                x=bbox_pred_reshape, shape=(0, -1, 4), name='bbox_pred_reshape_fpn{}'.format(i+3))
