...
class RetinaNet(object):
    ...
    def eval_bbox(self):
        ...
        '''
        decoded_box = fluid.layers.box_coder(
            prior_box=anchor_list,
            prior_box_var=var_list,
            target_box=boxes_list,
            code_type='decode_center_size',
            box_normalized=False)

        im_scale = fluid.layers.slice(self.im_info, [1], starts=[2], ends=[3])
        decoded_box = decoded_box / im_scale
        cliped_box = fluid.layers.retina_box_clip(input=decoded_box, im_info=self.im_info)
        scores_list = fluid.layers.transpose(scores_list, perm=[0, 2, 1])
