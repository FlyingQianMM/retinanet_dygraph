...
__all__ = [
    ...
    'linear_lr_warmup', 'piecewise_decay_with_linear_lr_warmup'
]
...
def piecewise_decay_with_linear_lr_warmup(boundaries, values, warmup_iter, start_lr, end_lr):
    """
    piecewise_decay_with_linear_lr_warmup
    """
    with default_main_program()._lr_schedule_guard():
        if len(values) - len(boundaries) != 1:
            raise ValueError("len(values) - len(boundaries) should be 1")
        ...
        if imperative_base.enabled():
            decay = imperate_lr.PiecewiseDecayWithLinearLrWarmup(boundaries, values,
                                                           warmup_iter, start_lr,
                                                           end_lr, 0)
            return decay
        else:
            global_step = _decay_step_counter()
