...
__all__ = [
    ...
    'PiecewiseDecayWithLinearLrWarmup'
]
...
class PiecewiseDecayWithLinearLrWarmup(LearningRateDecay):

    def __init__(self, boundaries, values, warmup_iter,
                 start_lr, end_lr, begin=1, step=1, dtype='float32'):
        super(PiecewiseDecayWithLinearLrWarmup, self).__init__(begin, step, dtype)
        self.boundaries = boundaries
        self.values = values
        self.warmup_iter = warmup_iter
        self.start_lr = start_lr
        self.end_lr = end_lr
        self.linear_step = float(end_lr) - float(start_lr)
    ...
    def step(self):
        if self.step_num < self.warmup_iter:
            decayed_lr = self.start_lr + self.linear_step * (self.step_num /
